function odf = dtr1d_4d_fit2podf(mfs_fn, odf_fn, opt)
% function odf = dtr1d_4d_fit2odf(mfs_fn, odf_fn, opt)
...
ODindex = opt.dtr1d.odf_watsonkappa; %Watson distribution smoothing kernel
odf_s.kappa = 1/tan(ODindex*pi/2);

    function [odf_w, odf_diso, odf_sddelta, odf_r1] = dtds2podf(odf_s, dtds)
        odf_w = zeros(sz(1), sz(2), sz(3), odf_s.n);
        odf_diso = zeros(sz(1), sz(2), sz(3), odf_s.n);
        odf_sddelta = zeros(sz(1), sz(2), sz(3), odf_s.n);
        odf_r1 = zeros(sz(1), sz(2), sz(3), odf_s.n);
...
%                        if n>0
                            ...
                            r1_vox = squeeze(dtds.r1(ni,nj,nk,:));
                            odf_d.x = -sin(theta_vox).*cos(phi_vox); % FLIP IN X HERE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            odf_d.y = sin(theta_vox).*sin(phi_vox);
                            odf_d.z = cos(theta_vox);
                            odf_d.w = w_vox;
                            odf_d.diso = diso_vox;
                            odf_d.sddelta = sddelta_vox;
                            odf_d.sqddelta = odf_d.sddelta;
                            odf_d.r1 = r1_vox;
                            odf_vox = dist_podf_discrete2smooth_R1(odf_d,odf_s);
                            odf_w(ni,nj,nk,:) = odf_vox.w(:);   
                            odf_diso(ni,nj,nk,:) = odf_vox.diso(:);   
                            odf_sddelta(ni,nj,nk,:) = odf_vox.sddelta(:);   
                            odf_r1(ni,nj,nk,:) = odf_vox.r1(:);   
%                        end
                     end
                end
            end
...
for nbin = 1:numel(opt.dtr1d.bin_disomax)
    ...
    ind_bin(:,:,:,:,3) = sddelta >= opt.dtr1d.bin_sddeltamin(nbin);
    ind_bin(:,:,:,:,4) = sddelta <= opt.dtr1d.bin_sddeltamax(nbin);
    ind_bin = all(ind_bin,5);

    odf_bin.no = nbin;
    dtds_temp = dtds;
    dtds_temp.w = dtds.w.*ind_bin;
    [odf_w, odf_diso, odf_sddelta, odf_r1] = dtds2podf(odf_s, dtds_temp);
    odf.w_bin{nbin} = odf_w;
    odf.diso_bin{nbin} = odf_diso;
    odf.sddelta_bin{nbin} = odf_sddelta;
    odf.r1_bin{nbin} = odf_r1;
end
