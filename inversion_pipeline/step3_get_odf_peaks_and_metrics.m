function step3_get_odf_peaks_and_metrics(input_parameters)
...
if ~strcmp(input_parameters.mrtrix_directory, '')
    ...
    if strcmp(method,'dtr2d')
        struct_sf_to_sh.r = odf_bsmedian.r2_bin{1};
        struct_sf_to_sh.t = odf_bsmedian.t2_bin{1};
    elseif strcmp(method,'dtr1d')
        struct_sf_to_sh.r = odf_bsmedian.r1_bin{1};
        struct_sf_to_sh.t = odf_bsmedian.t1_bin{1};
    end
    sf_to_sh(struct_sf_to_sh, 8, [0 1 0], mask_fn, odf_directory, method) % Permanent flip in y to account for mrtrix flip
...
if FLAG_relaxation
    parfor v = 1:N        
        ...
        if strcmp(method,'dtr2d')
            odf_s.r = squeeze(odf_bsmedian.r2_bin(v,:))';
            odf_s.t = squeeze(odf_bsmedian.t2_bin(v,:))';
        elseif strcmp(method,'dtr1d')
            odf_s.r = squeeze(odf_bsmedian.r1_bin(v,:))';
            odf_s.t = squeeze(odf_bsmedian.t1_bin(v,:))';
        end
        ...
        if strcmp(method,'dtr2d')
            structure.r2_peaks = r_peaks;
            structure.t2_peaks = t_peaks;
        elseif strcmp(method,'dtr1d')
            structure.r1_peaks = r_peaks;
            structure.t1_peaks = t_peaks;
        end
