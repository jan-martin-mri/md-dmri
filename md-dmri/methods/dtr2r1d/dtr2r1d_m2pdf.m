function dtr2r1d_m2pdf(dps_fn, pdf_path, opt)
% function dtr2r1d_m2pdf(dps_fn, pdf_path, opt)
        ...
        for ni = ni_range
            if dps.mask(ni,nj,nk)
                ...
                if s0 >= s0_threshold*s0max
                    ...
                    [n,par,perp,theta,phi,r2,r1,w] = dtr2r1d_dist2par(dtr2r1d);
                    if n>0
...
% eval(['print ' pdf_path '/dtr2r1d_2Dmap -loose -dpdf'])
eval(['print ' pdf_path '/dtr2r1d_2Dmap -loose -dpng -r300'])
pause(1.5)
...
eval(['print ' pdf_path '/dtr2r1d_global -loose -dpdf'])
